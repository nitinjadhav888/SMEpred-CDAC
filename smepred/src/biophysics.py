...
def calculate_nuclease_penalty(
    ...
    Returns:
        float: The penalty score (0.0 to 16.0).
    """
    total_penalty = 0.0

    # Evaluate PS backbone coverage
...
def calculate_risc_penalty(
    ...
    for i in range(5, min(8, len(antisense))):
        if antisense[i] == "8":
            total_penalty -= 2.0  # Therapeutic bonus
    ...
    return min(max(total_penalty, -10.0), 60.0)
...
def calculate_adjusted_efficacy(
    ...
    """
    penalties = {
        "nuclease": calculate_nuclease_penalty(sense, antisense, base_sense, base_antisense),
        "immuno": calculate_immuno_penalty(sense, antisense, base_sense, base_antisense),
        "risc": calculate_risc_penalty(sense, antisense, base_sense, base_antisense),
        "thermo": calculate_thermo_penalty(sense, antisense, base_sense, base_antisense),
        "serum": calculate_serum_penalty(sense, antisense, base_sense, base_antisense),
    }
    
    absolute_penalty_sum = sum(penalties.values())
